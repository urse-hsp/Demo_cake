        ...
        {this.state.Goods.length >= 1 && (
          <div>
            ...
            <ul>
              {this.state.Goods &&
                this.state.Goods.map((item, index) => {
                  return (
                    <li key={index} className={CSS.MtGoods}>
                      ...
                      <div className={CSS.details}>
                        ...
                        <div className={CSS.quantity}>
                          <button
                            className={item.num == 1 ? CSS.subtract : ""}
                            onClick={this.changNum.bind(this, 0, index, item)}
                          ></button>
                          <span>{item.num}</span>
                          <button
                            className={CSS.plus}
                            onClick={this.changNum.bind(this, 1, index)}
                          ></button>
                        </div>
                      </div>
                      {item.classify && (
                      ...
                      <div className={CSS.pitchOnWrap}>
                        <i
                          className={
                            item.Boole
                              ? CSS.pitchOn + " " + CSS.pitchOnActive
                              : CSS.pitchOn
                          }
                          onClick={this.pitchOn.bind(this, index)}
                        ></i>
                      </div>
                    </li>
                  );
            ...
            <div className={CSS.wrap}>
              <div>
                <span
                  className={this.props.checkAll ? CSS.checkAll : ""}
                  onClick={this.checkAll}
                ></span>
                全选
              </div>
              <div>
                应付：￥<em>{this.props.inAll}</em>
                <button>结算</button>
              </div>
            </div>
            <section className={CSS.mountings}>
              <h2>精品配件</h2>
              <ul>
                {this.state.market.mountings &&
                  this.state.market.mountings.map((item, index) => {
                    return (
                      <li
                        key={index}
                        className={CSS.mountingsGoods}
            ...
            <section className={CSS.essential}>
              <h2>派对必备 欢喜加倍</h2>
              <ul>
                {this.state.market.essential &&
                  this.state.market.essential.map((item, index) => {
                    return (
                      <li
                        key={index}
                        className={CSS.essentialGoods}
                        onClick={this.join.bind(this, item)}
                      >
                        <img src={item.goodsImg} />
                        <p>{item.goodsName}</p>
            ...
            <section className={CSS.overflow}>
              <h2>
                <a name="overflow">超值组合专享礼</a>
              </h2>
              <ul>
                {this.state.market.overflow &&
                  this.state.market.overflow.map((item, index) => {
                    return (
                      <li
                        key={index}
                        className={CSS.overflowGoods}
                        onClick={this.join.bind(this, item)}
                      >
                        <img src={item.goodsImg} />
                        <p>{item.goodsName}</p>
            ...
            <ul className={CSS.recommendList}>
              {this.state.recommend &&
                this.state.recommend.map((item, index) => {
                  return (
                    <li key={index} className={CSS.goods}>
                      <Link to={"/goodsDetail/" + item.ID}>
                        <img src={item.goodsImg} />
                        <p>{item.goodsName}</p>
                        <div>
